...
fn make_animal_talk(animal: &(impl Animal)){ 
    ...
    println!("imprimimos lo correspondiente implementado en rl trait {}", animal.talk()); 
}
...
fn ret_val_box(var: bool)->Box<dyn Animal>{
    if var {
        Box::new(Dog{name:"Box_terri".to_string()})
    }else{
        Box::new(Spider{name: "man".to_string()})
    }
}
...
struct Coordinate{ // with this we can pass an I32 or f64
    longitude: f64,
    latitude: f64, 
}
...
impl Add for Coordinate {
    ...
    fn add(self, other: Coordinate) ->Coordinate { //rhs = right hand side
        Coordinate { longitude: (self.longitude + other.longitude), 
...
fn main(){
    ...
    assert_eq!(Coordinate{latitude:1.0, longitude: 2.0} + Coordinate {latitude:3.0, longitude:4.0}, 
        Coordinate {longitude: 6.0, latitude: 4.0})
    //SUPERTRAITS ...
