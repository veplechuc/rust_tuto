...
fn main(){
    let x =5;
    let y = x; // here is a copy happening BECAUSE IT ON THE STACK AND WORKS WITH INT, BOOL, CHAR
...
// second example
fn from_reference(some_var: &String) -> usize { // esta funcion hace un borrow de s pero no se puede cambiar s
...
fn dangle() -> &String{ //<<<--- CHECK FOR THE ERROR THAT HAPPENS HERE
